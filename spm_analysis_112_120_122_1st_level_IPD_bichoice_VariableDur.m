%bichoice model with variable duration based on ChoiceRT
...
for i=1:length(IDs)
    ...
    subsname = dir(IDs{i});
     if (strcmp(subsname(1,1).name(1:3),'Sub') && subsname(1,1).isdir==1)  
         ...
         cd(open_path);         
         new_fold='1st_level_analysis_biChoice_2sessions_VariableDur';
         ...
         if isempty(dir([outpath 'SPM.mat']));
             cd(TemplateFolder) %need templates for unique subjs 
                ...
                load 112_120_122_1st_level_IPD_Choice_SRSD_joint_analysis_VariableDur
...
%            end
           ...
           if run==1 %SR anti
                ...
                if ~isnan(SR_ANTI_onsets{1})
                    ...
                    %matlabbatch{1,1}.spm.stats.fmri_spec.sess(1).cond(1).duration= 6;
                    matlabbatch{1,1}.spm.stats.fmri_spec.sess(1).cond(1).duration= SR_run_002_Coop_RTs;
                else
                    matlabbatch{1,1}.spm.stats.fmri_spec.sess(1).cond(1).onset= NaN(1,1);
                    %matlabbatch{1,1}.spm.stats.fmri_spec.sess(1).cond(1).duration= 6;
                end;                
               ...
               %DefectChoice onsets/durations
                if ~isnan(SR_ANTI_onsets{2})
                    ...
                    matlabbatch{1,1}.spm.stats.fmri_spec.sess(1).cond(2).duration= SR_run_002_Defect_RTs;
                else
...
if run==2 %SD ANTI - for 112, 120, 122
                ...
                if ~isnan(SD_ANTI_onsets{1})
                    matlabbatch{1,1}.spm.stats.fmri_spec.sess(2).cond(1).onset= SD_ANTI_onsets{1};
                    %matlabbatch{1,1}.spm.stats.fmri_spec.sess(2).cond(1).duration= 6;
                    matlabbatch{1,1}.spm.stats.fmri_spec.sess(2).cond(1).duration= SD_run_002_Coop_RTs;
                else
                ...
                if ~isnan(SD_ANTI_onsets{2})
                    matlabbatch{1,1}.spm.stats.fmri_spec.sess(2).cond(2).onset= SD_ANTI_onsets{2};
                    %matlabbatch{1,1}.spm.stats.fmri_spec.sess(2).cond(2).duration= 6;
                    matlabbatch{1,1}.spm.stats.fmri_spec.sess(2).cond(2).duration= SD_run_002_Defect_RTs;
                else 
...
if FLAG
        ...
        if isempty(dir([outpath 'con*']))
           ...
           SPM = add_contrasts_IPD_2sessions_bichoice_basic(SPM, size(R,2), subsname(1,1).name) %CHECK ME<
        else
