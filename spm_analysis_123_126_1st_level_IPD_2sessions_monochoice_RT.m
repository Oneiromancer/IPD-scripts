...
%monochoice model variable Choice duration based on RT 
%exclude SR PRO for 123 and 126
     ...
     if (strcmp(subsname(1,1).name(1:3),'Sub') && subsname(1,1).isdir==1)  
         ...
         if isempty(dir([outpath 'SPM.mat']));
             cd(TemplateFolder) %need templates for unique subjs 
                ...
                load 123_126_1st_level_IPD_Choice_SRSD_joint_analysis_monoChoice_RT
...
%            end
           ...
           if run==1 %SR anti
                matlabbatch{1,1}.spm.stats.fmri_spec.sess(1).cond(1).onset=' '; %ajk edited to sess(1)
...
if run==2 %SD PRO - for 123,126
                ...
                if ~isnan([SD_PRO_onsets{1}; SD_PRO_onsets{2}]);
                    matlabbatch{1,1}.spm.stats.fmri_spec.sess(2).cond(1).onset= [SD_PRO_onsets{1}; SD_PRO_onsets{2}];
                    %matlabbatch{1,1}.spm.stats.fmri_spec.sess(2).cond(1).duration= 6;
                    %ChoiceType ParaMod vector 
                    matlabbatch{1,1}.spm.stats.fmri_spec.sess(2).cond(1).pmod.param=[ones(length(SD_PRO_onsets{1}),1); -1*ones(length(SD_PRO_onsets{2}),1)];
                else
...
matlabbatch{1,1}.spm.stats.fmri_spec.sess(3).cond(1).duration = [SD_run_002_Coop_RTs;SD_run_002_Defect_RTs];
