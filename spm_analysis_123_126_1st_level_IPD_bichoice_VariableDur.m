...
%bichoice_vardur exclude SR-PRO (Subs 123, 126)
     ...
     if (strcmp(subsname(1,1).name(1:3),'Sub') && subsname(1,1).isdir==1)  
         ...
         if isempty(dir([outpath 'SPM.mat']));
             cd(TemplateFolder) %need templates for unique subjs 
                ...
                load 123_126_1st_level_IPD_Choice_SRSD_joint_analysis_VariableDur
...
if run==2 %SD PRO - for 123,126
                ...
                if ~isnan(SD_PRO_onsets{1})
                    ...
                    matlabbatch{1,1}.spm.stats.fmri_spec.sess(2).cond(1).duration= SD_run_001_Coop_RTs;
                else
                ...
                if ~isnan(SD_PRO_onsets{2})
                    ...
                    matlabbatch{1,1}.spm.stats.fmri_spec.sess(2).cond(2).duration= SD_run_001_Defect_RTs;
                else 
