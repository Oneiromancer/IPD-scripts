...
for i=1:length(IDs)
    ...
    subsname = dir(IDs{i});
     if (strcmp(subsname(1,1).name(1:3),'Sub') && subsname(1,1).isdir==1)  
         ...
         cd(open_path);         
        new_fold='1st_level_analysis_Choice_2sessions_ARTTHRESH_CompCor';
...
%                                 cd(file_path);
            ...
            disp(['No SPM file found on subject_' subsname(1,1).name '\' session_name '!']);
            end
...
if FLAG
        ...
        if isempty(dir([outpath 'con*']))
           ...
           SPM=add_contrasts_IPD_2sessions_CHOICE(SPM,16);
        else
