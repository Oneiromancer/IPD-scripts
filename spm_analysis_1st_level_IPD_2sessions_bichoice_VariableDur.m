%BICHOICE Variable_Dur based on ChoiceRT
...
for i=1:length(IDs)
    ...
    if ismember(subsname.name, Excl_SDPRO_IDs) 
        spm_analysis_103_1st_level_IPD_bichoice_VariableDur
    elseif ismember(subsname.name, Excl_SRANTI_IDs)
        spm_analysis_107_118_124_1st_level_IPD_bichoice_VariableDur
    elseif ismember(subsname.name, Excl_SRSDPRO_IDs)
        spm_analysis_112_120_122_1st_level_IPD_bichoice_VariableDur
    elseif ismember(subsname.name, Excl_SRPRO_IDs)
        spm_analysis_123_126_1st_level_IPD_bichoice_VariableDur
    elseif ismember(subsname.name, Excl_SDANTI_IDs)
        spm_analysis_113_1st_level_IPD_bichoice_VariableDur
     ...
     if (strcmp(subsname(1,1).name(1:3),'Sub') && subsname(1,1).isdir==1)  
        ...
        new_fold='1st_level_analysis_biChoice_2sessions_VariableDur'; %include R_16 by default
         ...
         if isempty(dir([outpath 'SPM.mat']));
            cd(TemplateFolder)
            load 1st_level_IPD_Choice_SRSD_joint_analysis_VariableDur
           ...
           if run==1 %SR PRO
               ...
               %CoopChoice onsets/durations
                if ~isnan(SR_PRO_onsets{1})
                    ...
                    %ajk commented -- vartiable durations added below
                    ...
                    %matlabbatch{1,1}.spm.stats.fmri_spec.sess(1).cond(1).duration= 6;
                    matlabbatch{1,1}.spm.stats.fmri_spec.sess(1).cond(1).duration= NaN(1,1);
                end;           
           ...
           if run == 3 %SD PRO
               ...
               %CoopChoice onsets/durations
                if ~isnan(SD_PRO_onsets{1})
                    ...
                    matlabbatch{1,1}.spm.stats.fmri_spec.sess(3).cond(1).duration= SD_run_001_Coop_RTs;
                else
                ...
                if ~isnan(SD_PRO_onsets{2})
                    ...
                    matlabbatch{1,1}.spm.stats.fmri_spec.sess(3).cond(2).duration= SD_run_001_Defect_RTs;
                else 
           ...
           if run == 4 %SD ANTI
               ...
               %CoopChoice onsets/durations
                if ~isnan(SD_ANTI_onsets{1})
                    ...
                    matlabbatch{1,1}.spm.stats.fmri_spec.sess(4).cond(1).duration= SD_run_002_Coop_RTs;
                else 
                ...
                if ~isnan(SD_ANTI_onsets{2})
                    ...
                    matlabbatch{1,1}.spm.stats.fmri_spec.sess(4).cond(2).duration= SD_run_002_Defect_RTs;
                else
